...
| 性质         | 描述                                                   | 数学表达                                                     |
| :----------- | :----------------------------------------------------- | :----------------------------------------------------------- |
| **有界性**   | 瑞利商的值被矩阵的最小和最大特征值所限定。             | $\lambda_{\min} \leq R(A, x) \leq \lambda_{\max}$<br />（由 Courant-Fischer 定理得出） |
...
因此我们发现，不断地迭代 $x_{k+1} = Ax_{k}$，最终 $x$ 收敛为最大特征值对应的特征向量
