...
> 这个式子的含义是：**$\nabla f$ 是由所有的 $\nabla h_{j}$ 线性组合而成的**（通过 Lagrange 乘子提供的比例）
...
???+ success "Definition：Lagrange 乘子法的一阶必要条件"
    ...
    \newline
    h_{j}(x^{\ast}) = 0 \quad (\text{即原等式约束成立})
    $$

我们记 $\mathcal{L}(x, \lambda) = f(x) + \sum_{j=1}^{n} \lambda_j h_j(x) $ 为 Lagrange 函数，上述的 $\nabla f(x^{\ast}) + \sum_{j=1}^n \lambda_j^{\ast} \nabla h_j(x^{\ast}) = 0$ 即为：
...
那么 KKT 条件和 Lagrange 乘子法的联系是什么呢？和 Lagrange 乘子法的一阶必要条件进行对照分析：
...
在 Lagrange 乘子法的基础上，新增了不等式约束的梯度项
...
在 Lagrange 乘子法的基础上，新增了不等式约束的约束项限制
...
不等式约束的拉格朗日乘子必须非负（因为不等式约束的搜索空间应该是向可行域内部的，不能反向），相比之下，等式约束的拉格朗日乘子没有符号限制（因为等式约束的搜索空间是整个切平面）
...
<1> 约束是紧的，$g_i(x^{\ast}) = 0$，此时最优解踩在这个约束的边界上，乘子 $\mu_{i}$ 可以不为 $0$

<2> 约束是松的，$g_i(x^{\ast}) < 0$，此时这个约束是没有实际限制意义的，对应的乘子 $\mu_{i}$ 必须为 $0$
...
???+ success "Definition：对偶函数"
    ...
    $$
    g(\lambda, ν) = \operatorname*{inf}_{x\in \mathcal{X}} \mathcal{L}(x, \lambda, ν)
    $$
    
    含义为：我们固定乘子 $\boldsymbol{\lambda}, \mathbf{v}$ 时，对 $x$ 求 $\mathcal{L}$ 的下确界
    
    **无论原问题是否是凸函数，对偶问题 $g(\lambda, ν)$ 一定是凹函数**
...
> $$
>
> 给出了原问题最优值 $p^\star$ 的一个**下界**（弱对偶性）
>
> $g(\lambda, \nu)$ 永远是凹函数，因此最大化它的**对偶问题**总是凸的。记对偶问题的最优值为  
>
> $$
> d^\star = \sup_{\lambda \ge 0,\, \nu} g(\lambda, \nu),
> $$
>
> 则有 $d^\star \le p^\star$。$d^\star$ 是原问题**最好的下界**
>
> 若 $d^\star = p^\star$，则称**强对偶性**成立

如果强对偶性成立，我们就可以通过解对偶问题去得到原问题的最优值
