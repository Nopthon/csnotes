...
不难发现在向上寻找根节点的过程中，路径上每个节点对应的根节点都是同一个
...
实际上，因为我们只考虑树形结构的连通性，正确性上并不需要区分哪个节点作为根节点 `dst`，但是为了降低并操作后树的深度（使得最坏时间复杂度更低），我们可以选择节点较少或深度较小的树作为 `src` 
