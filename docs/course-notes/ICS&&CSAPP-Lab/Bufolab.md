...
Task succeeded.
[1]    1914 segmentation fault  ./phase1 phase1.txt
```
...
--> 因为栈结构已经被破坏了，这是这一阶段执行溢出攻击的正常情况。当你劫持了返回地址并执行了特定函数之后，你就不需要再关心之后发生的事情了（比如通过返回的后门函数 Getshell，不需要知道 `system(sh)` 之后会发生什么，任务已经完成）

2- `message` 函数要求入口参数不为 `0`，但是上面的操作并没有修改相关寄存器的值？

--> 因为入口参数寄存器 `rdi` 的值在之前的程序运行过程中设置为非零值，并且在劫持返回地址的时候没有清理，因此满足入口参数非零的条件
...
opfunc3(str+13, '\0' );						/* 最终的 payload 偷了个懒，没有加 \0，因为之后的空数据正好为 0x00 */
    ↓
puts(str);
```
