...
比如 `main.c` 引用了 `test.c` 的函数 `add`，就需要将 `main.c` 和 `test.c` 中的 `add` 符号建立关联

2- 重定位：可重定位文件的代码区和数据区从地址 0 开始，在合并若干重定位文件后，得到的可执行文件满足 ABI 规范下的虚拟地址空间划分（比如只读代码段从 `0x8048000` 开始，可读写代码段从只读代码段后的第一个 4 KB 对其的地址处开始），因此需要重新确定代码和数据的地址，更新指令中被引用符号地址，这就是重定位
...
磁盘文件的各个可装入段信息向虚拟地址的映射如图所示，程序映像从 `0x8048000` 开始装填，内核地址从 `0xC0000000` 开始
