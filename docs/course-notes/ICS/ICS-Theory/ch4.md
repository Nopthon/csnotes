...
x86-64 对应的 AMD64 System V ABI 规定：栈中参数按 8 Bytes 的对齐，比如通过栈传递更多的参数时，1/2/4/8 Bytes 的参数在内存的起始地址必须是 8 的倍数

但是在存储非静态局部变量时，我们发现有时 8 Bytes 的单层栈中存入了多个变量，比如：
...
如果某个 8 Bytes 的数据不按照八字节宽的对齐方式进行存储，那么读取一个 8 Bytes 的数据就可能需要两次访存，使得指令的执行效率降低
...
结构体在内存中一定是按照声明顺序依次存储的，但为了对齐需要进行一些填充来实现对齐（和存储非静态局部变量不一样，后者不要求按照某种顺序存储）

对于 x86-64 System V ABI，结构体对齐满足下面的条件：

- 结构体**起始地址**对齐到其**最宽基本类型成员大小**的整数倍。
- 每个成员放在**其类型大小整数倍**的偏移地址上。
- 结构体总大小为其最宽成员大小的整数倍。
